...
write.table(cbind("Pathway","Beta","HR_with_CI","Concordance","P-value", "Total_path_genes" , "Path_sig_Yes", "Percentage", "p.value", "odd_ratio"),
            file=paste0(path,folder[c], "/", "All_Pathways_PI_Survival_results.txt"),row.names=F,col.names=F,sep = '\t');

for (j in 1:length(subfolder))
  {
tryCatch({

  folder1<-dir.create(paste0(path,folder[c], "/","All_", subfolder[j]))

a<- paste0(folder1, "/")
print(a)  
  
  setwd(paste0(path,folder[c], "/", "All_", subfolder[j]))
getwd()
...
######################################################
  write.table(cbind(paste0(subfolder[j]),coeff, HR1, CI, pval,path_Fisher_test),
              file=paste0(path,folder[c], "/", "All_Pathways_PI_Survival_results.txt"),row.names=F,col.names=F,sep = '\t',append = T);#output file
