...
neg_features <- subset(all_features_results, Beta < 0 )
...
write.table(cbind("Pathway","Beta","HR_with_CI","Concordance","P-value", "Total_path_genes" , "Path_sig_Yes", "Percentage", "p.value", "odd_ratio"),
            file=paste0(path,folder[c], "/", "Neg_Pathways_PI_Survival_results.txt"),row.names=F,col.names=F,sep = '\t');

for (j in 1:length(subfolder))
  {
tryCatch({

  folder1<-dir.create(paste0(path,folder[c], "/","Neg_", subfolder[j]))

a<- paste0(folder1, "/")
print(a)  
  
  setwd(paste0(path,folder[c], "/", "Neg_", subfolder[j]))
getwd()
...
write.table(cbind("ID"=rownames(features ), features ),file="features.txt",sep="\t",quote=F, row.names=F)
 ...
 # sel_features_results<-all_features_results[row.names(all_features_results) %in% row.names(features),]
  ...
  sel_features_results<-neg_features[row.names(neg_features) %in% row.names(features),] 
  dim(sel_features_results)
print("length_sel_ftr:");
  print(length(sel_features_results))
# if (length(sel_features_results) != 1 ){print("empty")}; next 
  
  sel_ftr_surv <- as.data.frame(sel_features_results[,1])
  names(sel_ftr_surv ) <- c("ID")
  head(sel_ftr_surv )
  dim(sel_ftr_surv )
# if (length(sel_ftr_surv) < 2 ) next
#if(length(sel_ftr_surv) < 2 ) {    print ('3rd iteration' ) ; next }  
  ...
  print("Length_sel_train:")
  
 print(length(sel_train))

#if (length(sel_features_results) == 0 ){print("empty")}; next 



#tryCatch({
 #   if (length(sel_train)==0) stop("No selected genes in pathway !")
...
######################################################
  write.table(cbind(paste0(subfolder[j]),coeff, HR1, CI, pval,path_Fisher_test),
              file=paste0(path,folder[c], "/", "Neg_Pathways_PI_Survival_results.txt"),row.names=F,col.names=F,sep = '\t',append = T);#output file
